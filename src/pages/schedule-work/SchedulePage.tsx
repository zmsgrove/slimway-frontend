...
function ShiftModal({ employee, date, existing, onClose, onSaved, onDeleted }: ShiftModalProps) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 21 }}>
      ...
      <div className="modal-animate" style={{ position: 'relative', width: '100%', maxWidth: 420, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}>

        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 13, marginBottom: 21, paddingBottom: 21, borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, paddingTop: 21, borderTop: '1px solid var(--border)', flexWrap: 'wrap' }}>
          ...
          {isDayOff ? (
            <button onClick={() => void handleSave()} disabled={saving}
              style={{ flex: 1, height: 36, background: 'var(--accent)', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1 }}>
              {saving ? '...' : 'Назначить смену'}
            </button>
          ) : (
            <button onClick={() => void handleSave()} disabled={saving}
              style={{ flex: 1, height: 36, background: 'var(--accent)', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1, minWidth: 90 }}>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 21 }}>
      ...
      <div className="modal-animate" style={{ position: 'relative', width: '100%', maxWidth: 480, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, boxShadow: '0 24px 64px rgba(0,0,0,0.5)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 21, paddingBottom: 21, borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)' }}>Быстрое назначение</div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {(['shift', 'dayoff'] as const).map(t => (
              <button key={t} onClick={() => setShiftType(t)}
                style={{ flex: 1, height: 36, borderRadius: 8, border: '1px solid', fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'background 150ms ease-out, border-color 150ms ease-out, color 150ms ease-out',
                  background: shiftType === t ? (t === 'dayoff' ? 'rgba(113,113,122,0.15)' : 'color-mix(in srgb, var(--accent) 12%, transparent)') : 'transparent',
                  borderColor: shiftType === t ? (t === 'dayoff' ? 'rgba(113,113,122,0.4)' : 'color-mix(in srgb, var(--accent) 40%, transparent)') : 'var(--border)',
                  color: shiftType === t ? (t === 'dayoff' ? 'var(--text-secondary)' : 'var(--accent)') : 'var(--text-muted)',
                }}>
        ...
        <div style={{ marginBottom: 21, paddingTop: 21, borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['weekdays', 'daily', 'every-n'] as RepeatType[]).map(r => (
              <button key={r} onClick={() => setRepeat(r)}
                style={{ flex: 1, height: 34, borderRadius: 8, border: '1px solid', fontSize: 11, fontWeight: 500, cursor: 'pointer', transition: 'background 150ms ease-out, border-color 150ms ease-out, color 150ms ease-out',
                  background: repeat === r ? 'color-mix(in srgb, var(--accent) 8%, transparent)' : 'transparent',
                  borderColor: repeat === r ? 'color-mix(in srgb, var(--accent) 35%, transparent)' : 'var(--border)',
                  color: repeat === r ? 'var(--accent)' : 'var(--text-muted)',
                }}>
            ...
            <div style={{ display: 'flex', gap: 5, marginTop: 10 }}>
              {WEEKDAY_LABELS.map((label, idx) => (
                <button key={idx} onClick={() => toggleWeekday(idx)}
                  style={{ flex: 1, height: 32, borderRadius: 7, border: '1px solid', fontSize: 11, fontWeight: 500, cursor: 'pointer', transition: 'background 150ms ease-out, border-color 150ms ease-out, color 150ms ease-out',
                    background: weekdaysMask[idx] ? 'color-mix(in srgb, var(--accent) 12%, transparent)' : 'transparent',
                    borderColor: weekdaysMask[idx] ? 'color-mix(in srgb, var(--accent) 40%, transparent)' : 'var(--border)',
                    color: weekdaysMask[idx] ? 'var(--accent)' : 'var(--text-muted)',
                  }}>
                  {label}
                </button>
              ))}
          ...
          {repeat === 'every-n' && shiftType === 'shift' && (
            <div style={{ marginTop: 10, padding: 10, background: 'color-mix(in srgb, var(--accent) 6%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 20%, transparent)', borderRadius: 8 }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 21, flexWrap: 'wrap', gap: 13 }}>
        <div>
          <h1 style={{ fontSize: 21, fontWeight: 600, color: 'var(--text)', margin: 0, marginBottom: 4 }}>График смен</h1>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: 0 }}>Расписание и учёт рабочего времени</p>
        </div>
        {canManage && (
          <button onClick={() => setBulkModal(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 8, height: 36, padding: '0 16px', background: 'var(--accent)', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>
      ...
      {/* Today panel */}
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 21, marginBottom: 13 }}>
      ...
      {/* Period navigation */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 13, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 13, padding: '8px 13px' }}>
        <button onClick={() => setMonday(m => addDays(m, -visibleDays))}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
          <div ref={gridContainerRef} style={{ overflowX: 'auto' }}>
            <table style={{ borderCollapse: 'collapse', tableLayout: 'fixed', width: '100%', minWidth: 160 + visibleDays * 52 }}>
              ...
              <thead>
                <tr>
                  <th style={{ padding: '10px 13px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap', position: 'sticky', left: 0, background: 'var(--bg-surface)', zIndex: 2 }}>
                    Сотрудник
                  </th>
                  {twoWeekDays.map(d => {
                    const iso     = toISO(d)
                    ...
                    return (
                      <th key={iso} style={{ padding: '5px 3px', textAlign: 'center', borderBottom: '1px solid var(--border)', borderLeft: '1px solid var(--border)', background: isToday ? 'color-mix(in srgb, var(--accent) 10%, transparent)' : wknd ? 'rgba(255,255,255,0.015)' : 'transparent', overflow: 'hidden' }}>
                        ...
                        <div style={{ fontSize: 9, color: isToday ? 'color-mix(in srgb, var(--accent) 70%, transparent)' : 'var(--text-muted)', lineHeight: 1.2, opacity: 0.8, whiteSpace: 'nowrap' }}>
              ...
              <tbody>
                {employees.map(emp => (
                  <tr key={emp.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '8px 13px', position: 'sticky', left: 0, background: 'var(--bg-surface)', zIndex: 1, borderRight: '1px solid var(--border)' }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 135 }}>{emp.full_name}</div>
                      ...
                      return (
                        <td
                          ...
                          style={{
                            ...
                            background: isToday ? 'color-mix(in srgb, var(--accent) 5%, transparent)' : wknd ? 'rgba(255,255,255,0.01)' : 'transparent',
                            cursor: canManage ? 'pointer' : 'default',
                            height: 50,
                            transition: 'background 0.12s',
                          }}
                          onMouseEnter={e => { if (canManage) (e.currentTarget as HTMLTableCellElement).style.background = isToday ? 'color-mix(in srgb, var(--accent) 10%, transparent)' : 'rgba(255,255,255,0.04)' }}
      ...
      <div style={{ display: 'flex', gap: 13, marginTop: 13, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <div style={{ width: 26, height: 14, borderRadius: 4, background: 'color-mix(in srgb, var(--accent) 15%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 45%, transparent)' }} />
